...
class NDArray:
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        new_offset = self._offset + sum([idxs[i].start * self.strides[i] for i in range(len(idxs))])
        new_shape = tuple([len(range(idxs[i].start, idxs[i].stop, idxs[i].step)) for i in range(len(idxs))])
        new_strides = tuple([idxs[i].step * self.strides[i] for i in range(len(idxs))])
        return NDArray.make(new_shape, device=self.device, handle=self._handle, strides=new_strides, offset=new_offset)
        ### END YOUR SOLUTION
