...
// Helper function to increment a number represented as a vector of digits.
// The difference is that each digit can be in a different base, so the number
// is represented as a vector of digits in different bases.
void _increment_digits(std::vector<uint32_t>& digits, const std::vector<uint32_t>& bases) {
...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<uint32_t> shape,
  ...
  // Create a vector of indices for each dimension, initialized to zero.
  std::vector<uint32_t> stride_indices(shape.size(), 0);

  // Iterate over all elements in the output array.
  for (size_t i = 0; i < out->size; i++) {
    size_t idx = offset;
    for (size_t j = 0; j < shape.size(); j++) {
      idx += (size_t)(stride_indices[j] * strides[j]);
    }
    out->ptr[i] = a.ptr[idx];

    // Increment the indices.
    _increment_digits(stride_indices, shape);
  }
  /// END YOUR SOLUTION
}
...
void EwiseDiv(const AlignedArray& a, const AlignedArray& b, AlignedArray* out) {
  /**
   * Set entries in out to be the quotient of corresponding entries in a and b.
   */
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = a.ptr[i] / b.ptr[i];
  }
...
void EwiseGe(const AlignedArray& a, const AlignedArray& b, AlignedArray* out) {
  /**
   * Set entries in out to be the greater than or equal of corresponding entries in a and b.
   */
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = (a.ptr[i] >= b.ptr[i]) ? 1 : 0;
  }
...
void ScalarGe(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  /**
   * Set entries in out to be the greater than or equal of corresponding entry in a and the scalar val.
   */
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = (a.ptr[i] >= val) ? 1 : 0;
  }
...
void EwiseLog(const AlignedArray& a, AlignedArray* out) {
  /**
   * Set entries in out to be the log of corresponding entry in a.
   */
  for (size_t i = 0; i < a.size; i++) {
...
void EwiseExp(const AlignedArray& a, AlignedArray* out) {
  /**
   * Set entries in out to be the exp of corresponding entry in a.
   */
  for (size_t i = 0; i < a.size; i++) {
...
void EwiseTanh(const AlignedArray& a, AlignedArray* out) {
  /**
   * Set entries in out to be the tanh of corresponding entry in a.
   */
  for (size_t i = 0; i < a.size; i++) {
