...
__device__ size_t IndexToLoc(const CudaVec shape, const CudaVec strides, size_t offset, size_t i) {
  /**
  ...
  for (size_t j = shape.size-1; j >= 0; j--) {
    size_t stride = strides.data[j];
    size_t dim = shape.data[j];
    size_t idx = i % dim;
    loc += idx * stride;
    i /= dim;
    if (j == 0) break;
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    size_t loc = IndexToLoc(shape, strides, offset, gid);
    out[gid] = a[loc];
  }
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                                   CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the EwiseSetitem operation.  This should effectively map a single entry in 
   * the non-compact input a, to the corresponding item (at location gid) in the non-compact array 
   * out.
   * 
   * Args:
   *   a: CUDA pointer to a array
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape,
                                    CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the ScalarSetitem operation.  This should effectively map a single entry in 
   * the non-compact input a, to the corresponding item (at location gid) in the non-compact array 
   * out.
   * 
   * Args:
